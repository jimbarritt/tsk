...
| T-19 | Add scheduled repo cleaning agent | Raised by Jim, 2026-09-19, from a live test against M-ADMIN-03 (dependency vulnerability watch). A Routine created with `create_trigger` fired a session missing every `mcp__github__*` tool: the tool warned on creation that it stores no MCP connectors and had none to pass through, and the fired session's own tool list confirmed it. Root cause, from Anthropic's Routines docs: the web UI's routine creation has an explicit repository-selection step that provisions GitHub access for the run; `create_trigger` has no equivalent field. So a scheduled agent that needs to read or act on a repo (Dependabot alerts, a cleanup pass, anything beyond plain `git` over the environment's own credentials) cannot be wired up through `create_trigger` alone today. Needs either recreating the routine from claude.ai/code/routines directly, or a mechanism this harness can invoke programmatically that grants the same access. M-ADMIN-03 is the first mission blocked on this | none | none | TODO |
