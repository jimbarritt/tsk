...
| T-23 | Fix `thread_bind_cloud`'s non-atomic write | Found during T-22's research, 2026-09-21. `thread_bind_cloud` (`thread-lib.sh`) writes via `mktemp` then `mv`, but `mktemp` lands in `$TMPDIR`, likely a different filesystem from `$WT`, so the rename degrades to copy-plus-unlink rather than an atomic replace. Fix: `mktemp -p "$(dirname "$lookup")"`. Applied and verified, 2026-09-21: tested in an isolated scratch worktree, produced the correct lookup entry with no stray temp file left behind. Pushed to `main`: `3cb635b` | none | none | DONE |
| T-24 | Check a continuation entry's recorded commits are reachable on `origin` | Found during T-22's research, 2026-09-21. `thread-append-handover.sh`'s `COMMIT_ON_MAIN` records the local `HEAD`, which can be unpushed or not on `main` at all; a different actor resuming elsewhere cannot see that commit. No invariant currently checks either recorded hash is reachable on `origin` before or after writing the entry. `commit_on_bootstrap` needed no fix: `thread_refresh_wt` resets `$WT` to origin's own fetched tip just before it is captured, so it is already on origin by construction. `commit_on_main` did: `thread-append-handover.sh` now fetches `origin/main` and refuses to write the entry (exits 1 with a clear message) unless `commit_on_main` is reachable on it. Applied and verified, 2026-09-21, in an isolated scratch repo: correctly refused an unpushed commit, correctly passed once it was pushed. Pushed to `main`: `3cb635b` | none | none | DONE |
